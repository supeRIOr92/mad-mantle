# data_sources/agni.py — Agni Finance (UniV3 fork) data source
# MIGRATED: subgraph → direct RPC via web3.py
# Detection: Z-Score + Bollinger (same as Fluxion)
#
# Verified onchain:
...
FACTORY_ADDRESS = Web3.to_checksum_address("0x25780dc8Fc3cfBD75F33bFDAB65e969b603b2035")
BUCKET_SIZE = 15 * 60
MANTLE_BLOCK_TIME = 2
...
def _get_token_meta(address: str) -> dict:
    ...
    try:
        ...
        logger.debug("[agni] token meta failed %s: %s", address, e)
...
def _ensure_pool_registry():
    ...
    try:
        ...
        logger.info("[agni] registry loaded — %d pools", len(_pool_registry))
        for addr, p in _pool_registry.items():
            logger.info("[agni] pool: %s (%s/%s)", addr, p["token0Symbol"], p["token1Symbol"])
    except Exception as e:
        logger.error("[agni] registry fetch failed: %s", e)
...
def _calc_amount_usd(amount0: int, amount1: int, meta: dict) -> float:
    ...
    if token0 in MANTLE_STABLECOINS:
        return abs(amount0) / (10 ** dec0)
    if token1 in MANTLE_STABLECOINS:
...
def _decode_swap_log(log: dict) -> dict | None:
    try:
        ...
        logger.warning("[agni] decode failed: %s", e)
        return None
...
def fetch_volume_buckets(pool_id: str, since_ts: int) -> list:
    ...
    try:
        ...
        logger.error("[agni] fetch_volume_buckets failed %s: %s", pool_id, e)
        return []
...
def fetch_daily_snapshots(pool_id: str, days: int = 7) -> list:
    ...
    try:
        ...
        logger.error("[agni] fetch_daily_snapshots failed %s: %s", pool_id, e)
        return []
