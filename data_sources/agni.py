# data_sources/agni.py — Agni Finance data source
# Queries unified custom subgraph for Agni swap data
...
QUERY_RECENT_SWAPS = gql("""
    query AgniRecentSwaps($since: BigInt!, $first: Int!) {
        swaps(
            where:          { dex: "agni", timestamp_gte: $since }
            orderBy:        timestamp
            orderDirection: desc
            first:          $first
        ) {
...
QUERY_VOLUME_BUCKETS = gql("""
    query AgniBuckets($poolId: String!, $since: BigInt!) {
        volumeBuckets(
            where:          { pool: $poolId, dex: "agni", bucketStart_gte: $since }
            orderBy:        bucketStart
            orderDirection: asc
        ) {
            bucketStart
...
QUERY_DAILY_SNAPSHOTS = gql("""
    query AgniDailySnapshots($poolId: String!, $days: Int!) {
        dailyPoolSnapshots(
            where:          { pool: $poolId, dex: "agni" }
            orderBy:        date
            orderDirection: desc
            first:          $days
        ) {
...
def fetch_recent_swaps(since_ts: int, limit: int = 500) -> list:
    """Fetch recent Agni swaps since timestamp."""
    try:
        client = get_client()
        ...
        logger.error(f"[agni] fetch_recent_swaps failed: {e}")
        return []
...
def fetch_volume_buckets(pool_id: str, since_ts: int) -> list:
    ...
    try:
        ...
        logger.error(f"[agni] fetch_volume_buckets failed: {e}")
        return []
...
def fetch_daily_snapshots(pool_id: str, days: int = 7) -> list:
    ...
    try:
        ...
        logger.error(f"[agni] fetch_daily_snapshots failed: {e}")
        return []


def fetch_top_pools(limit: int = 20) -> list:
    """Fetch top Agni pools by volume — used for pool discovery."""
    try:
        client = get_client()
        result = client.execute(QUERY_TOP_POOLS, variable_values={"first": limit})
        return result.get("pools", [])
    except Exception as e:
        logger.error(f"[agni] fetch_top_pools failed: {e}")
        return []